        ...
        <div className="display-area-container flex-c-col">
            <div className={getClassName(page)}>
                <div className={(workshops.includes(selectedData.name) || !staffEditPermissions) ? "summary-card-header-center flex-c" : "summary-card-header flex-c"}>
                    ...
                    {!workshops.includes(selectedData.name) && (staffEditPermissions || currentUser.user === selectedData.name) && page === "staff" && <Tooltip content="Edit Employee" xPos="-15px" yPos="-43px" btnTranslateX="-20px" ButtonComponent={ButtonComponent} onClick={() => openAddUpdateForm(setAddUpdateFormVisible)} />}
                    {equipmentEditPermissions && page === "technical-info" && <Tooltip content="Edit Device" xPos="-10px" yPos="-43px" btnTranslateX="-20px" ButtonComponent={ButtonComponent} onClick={() => showDeviceUpdate(setUpdateFormVisible)} />}
                </div>
