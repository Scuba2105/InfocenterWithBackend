...
const staffInputsRegexArray = [/^[a-z ,.'-]+$/i, //Contact
                               /^[a-z &/]+$/i, // Position
                               /^[0-9]{10}$|^[1-9][0-9]{7}$|^[0-9]{5}$|^[0-9]{2}\s[0-9]{4}\s[0-9]{4}$|^[0-9]{2}\s[0-9]{3}\s[0-9]{3}$|^\s*$/, // Office Phone (allows empty string) 
                               /^[0-9]{5}$|^\s*$/, // Dect Phone
...
async function uploadUpdatedDetails(idNumber, currentContact, formType, formContainer, updatedContactData, queryClient, showMessage, closeDialog, closeUpdateContactModal) {
    delayFunctionInitiation(async () => {
    ...
    return (
        <div className="contact-update-display flex-c-col">
            ...
            <div className="form-buttons">
                <FormButton content="Upload" btnColor="#D4FB7C" marginBottom="0px" marginTop="0px" onClick={() => uploadUpdatedDetails(currentContact.id, currentContact.data, formType, formContainer, updatedContactData, queryClient, showMessage, closeDialog, closeUpdateContactModal)} /> 
            </div>
        </div>
    )
}