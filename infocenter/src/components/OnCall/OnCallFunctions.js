...
function getClassList(entry, currentUser) {
                ...
                if (modalLinkButtons.includes(type)) {
                    return (
                        <div key={`${entry.label}-key`} className="on-call-function-container flex-c size-100" style={isFeatureAccessible(entry, currentUser) ? {opacity: 1} : {opacity: 0.2}}>
                            <button className={isFeatureAccessible(entry, currentUser) ? `on-call-function-button flex-c-col ${type} ${design}` : `on-call-function-button inactive flex-c-col ${type} ${design}`} onClick={isFeatureAccessible(entry, currentUser) ? () => showForm(setFormVisibile, setFormType, type) : null }>
